...
public class Chronographe<T extends Stopwatch> extends BaseChronographe implements SurfaceHolder.Callback, SurfaceViewRenderer<T> {
    ...
    private void renderOnCacheHeadLine1(T stopwatch, Pwrapper<Integer> blBottomVPos, boolean replace) {
        ...
        if (replace) {
            rect.set(0, blBottomVPos.value(), getScreenWidth(), blBottomVPos.value() + normalTextBlocHeight);
        ...
        cachedCanvas.drawText(stopwatch.getStopwatchData().getChronoType().toString(), getScreenWidth() - SPACING, blBottomVPos.value() + normalTextBaseLineVerticalOffset, paintNormalTextWhiteRigth);
    ...
    private void renderOnCacheHeadLine2(T stopwatch, Pwrapper<Integer> blBottomVPos, boolean replace) {
        ...
        cachedCanvas.drawText(stopwatch.getStopwatchData().getInfo(), SPACING, blBottomVPos.value() + normalTextBaseLineVerticalOffset, paintNormalTextWhiteLeft);
        blBottomVPos = blBottomVPos.add(normalTextBlocHeight);
    }

    private void renderOnCacheHeadLineDigit(T stopwatch, Pwrapper<Integer> blBottomVPos) {
        // third (or second line)
        rect.set(0, blBottomVPos.value(), getScreenWidth(), blBottomVPos.value() + digitTextBlocHeight);
        cachedCanvas.drawRect(rect, paintBackgroundBlack);
        cachedCanvas.drawText(stopwatch.getTime().toString(), getScreenWidth() - SPACING, blBottomVPos.value() + digitTextBaseLineVerticalOffset, paintWhiteRight);
        blBottomVPos = blBottomVPos.add(digitTextBlocHeight);
    }

    private void renderOnCacheHLine(T stopwatch, Pwrapper<Integer> blBottomVPos) {
        // sep line
        rect.set(0, blBottomVPos.value(), getScreenWidth(), blBottomVPos.value() + SPACING / 2);
        cachedCanvas.drawRect(rect, paintLine);
        blBottomVPos = blBottomVPos.add(SPACING / 2);
    }
    ...
    private void renderOnCacheDelete(T stopwatch, int offSet) {
        ...
        rect.set(0, offSet, getScreenWidth(), offSet + stopwatch.getStopwatchUi().getHeadHeight() + stopwatch.getStopwatchUi().getDetailsHeight());
        cachedCanvas.drawRect(rect, paintBackgroundBlack);
        ...
        for (UPDATE_TYPE updateType : stopwatch.getStopwatchUi().getUpdateTypes())
            switch (updateType) {
                case DELETE:
                    ...
                    break;
                case ADD_NEW:
                    renderOnCacheAddNew(stopwatch, offSet);
                    break;
    ...
    private void renderOnCacheRunning(T stopwatch, int i) {
        int offSet = getOffset(i);
        int blBottomVPos = offSet + stopwatch.getStopwatchUi().getHeadHeight();
        blBottomVPos -= SPACING / 2;
        rect.set(getScreenWidth() - SPACING - digitMaxLength, blBottomVPos - digitTextBlocHeight, getScreenWidth() - SPACING, blBottomVPos);
        cachedCanvas.drawRect(rect, paintBackgroundBlack);
        blBottomVPos -= digitTextBlocHeight;
        cachedCanvas.drawText(stopwatch.getTime().toString(), getScreenWidth() - SPACING, blBottomVPos + digitTextBaseLineVerticalOffset, paintWhiteRight);
    ...
    private class DialogListener implements OnDialogClickListener {
        ...
        public void onDialogPositiveClick(DialogFragment dialog) {
            ...
            if (cdialog.getType() != null) {
                if (touchedStopwatch.getStopwatchData().getChronoType() != cdialog.getType())
                    touchedStopwatch.getStopwatchUi().addUpdateType(CHANGE_TYPE);
                touchedStopwatch.getStopwatchData().setChronoType(cdialog.getType());
            }

            if (cdialog.getType() == Units.CHRONO_TYPE.PREDEFINED_TIMES || cdialog.getType() == Units.CHRONO_TYPE.INSIDE_LAP) {

                if (cdialog.getDistance() > 0) {
                    if (touchedStopwatch.getStopwatchData().getgLength() != cdialog.getDistance())
                ...
                if (cdialog.getSpeedUnit() != null) {
                    if (touchedStopwatch.getStopwatchData().getgLength()>0 && touchedStopwatch.getStopwatchData().getChronoTime()>0
                            && touchedStopwatch.getStopwatchData().getSpeedUnit() != cdialog.getSpeedUnit() )
                        touchedStopwatch.getStopwatchUi().addUpdateType(UPDATE_HEAD_LINE2);
